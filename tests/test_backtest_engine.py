...
def test_backtest_engine_records_equity_and_metrics():
    ...
    events = [Event(t, 100.0), Event(t + timedelta(seconds=1), 110.0), Event(t + timedelta(seconds=2), 105.0)]
    ...
    result = BacktestEngine(1000.0, fee_rate=0.0).run(events, strategy, lambda event: event.price)
