...
from aitos.backtest.rich_cli import auction_decision, _row_to_event
from aitos.models.market import OrderBookSnapshot, TradeTick


def test_auction_baseline_returns_flat_below_threshold():
    decision = auction_decision(SimpleNamespace(auction_long_score=0.4, auction_short_score=0.5))
    assert decision.direction == "flat"
    assert decision.quantity == 0


def test_auction_baseline_selects_stronger_side():
    decision = auction_decision(SimpleNamespace(auction_long_score=0.8, auction_short_score=0.3))
