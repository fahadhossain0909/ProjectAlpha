...
class FakeBus:
    ...
    async def subscribe(self, topic, handler, group="default"):
        self.subscriptions.append((topic, group, handler)); return FakeSubscription(handler)

@pytest.mark.asyncio
async def test_live_cache_rehydrates_trade_and_book_events():
    bus = FakeBus(); cache = LiveScannerCache(bus, ["BTCUSDT"], max_trades=10)
    ...
    await cache._on_trade(type("E", (), {"payload": trade.to_dict(), "topic": "market.trade.BTCUSDT"})())
    await cache._on_book(type("E", (), {"payload": book.to_dict(), "topic": "market.orderbook.BTCUSDT"})())
...
async def test_live_cache_keeps_bounded_trade_history():
    bus = FakeBus(); cache = LiveScannerCache(bus, ["BTCUSDT"], max_trades=100)
    ts = datetime.now(timezone.utc)
    for i in range(150):
