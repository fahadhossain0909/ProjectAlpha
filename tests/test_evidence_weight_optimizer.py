...
def perf(source, observations, alignment, avg_r):
    return EvidencePerformance(source, observations, int(observations * alignment), 10, 5, 10.0, avg_r, alignment, 0.66)


def test_optimizer_is_bounded_and_preserves_total_weight():
    base = {"amt": 0.10, "order_flow": 0.15, "liquidity": 0.10, "rl": 0.10}
    candidate = EvidenceWeightOptimizer(min_observations=20, max_relative_change=0.20).propose(
