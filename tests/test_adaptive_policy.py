...
def test_weak_regime_is_disabled_and_tightened():
    ...
    candidate = engine.propose(report(PerformanceSlice("regime", "range", 10, 3, 7, -2, -0.2, -0.2, 0.3)), "cand-1")
...
def test_strong_regime_can_lower_threshold_but_not_below_floor():
    engine = AdaptivePolicyEngine(min_trades=5)
    candidate = engine.propose(report(PerformanceSlice("regime", "trend", 10, 7, 3, 2, 0.2, 0.2, 0.7)), "cand-2")
...
def test_insufficient_sample_keeps_base_policy():
    ...
    candidate = engine.propose(report(PerformanceSlice("regime", "unknown", 3, 3, 0, 1, 0.3, 0.3, 1.0)), "cand-3")
