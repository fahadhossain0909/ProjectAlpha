...
def _events(days: int = 10):
    ...
    return [HistoricalEvent(start + timedelta(days=i), 100.0 + i, {"symbol": "BTCUSDT"}) for i in range(days)]
...
def test_experience_record_is_serializable_and_source_bound():
    record = ExperienceRecord(timestamp=datetime.now(timezone.utc), source="paper", symbol="BTCUSDT", decision="LONG", confidence=0.8)
...
def test_candidate_validator_uses_canonical_backtest():
    result = CandidateValidator(ValidationPolicy(min_trades=0)).evaluate(_events(), _strategy, lambda e: e.price)
...
def test_walk_forward_keeps_locked_holdout_out_of_windows():
    ...
    result = WalkForwardValidator(min_positive_windows=0).evaluate(events, _strategy, lambda e: e.price, start, end, train_days=180, test_days=30, holdout_days=30)
...
def test_model_registry_promotes_only_candidates(tmp_path):
    ...
    registry.register(ModelArtifact(name="strategy", version="v1", kind="rule", status="candidate"))
