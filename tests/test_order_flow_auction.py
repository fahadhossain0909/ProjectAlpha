...
def _trade(qty, buyer_maker=False):
    return TradeTick(symbol="BTCUSDT", trade_id=1, price=100.0, quantity=qty, side=TradeSide.BUY if not buyer_maker else TradeSide.SELL, is_buyer_maker=buyer_maker, timestamp=datetime.now(timezone.utc))
...
def test_auction_context_is_bounded():
    ...
    klines = [Kline("BTCUSDT", "15m", now, now, 100+i, 102+i, 99+i, 101+i, 100, 100, 10, 55, 55) for i in range(20)]
