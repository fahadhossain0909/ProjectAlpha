...
def make_opportunity(**overrides) -> Opportunity:
    defaults = dict(symbol="BTCUSDT", side=TradeSide.LONG, entry_price=100.0, stop_loss_price=98.0, take_profit_levels=[104.0], confidence=0.8, strategy_id="test-strategy", rationale="test rationale")
    defaults.update(overrides)
...
async def test_full_lifecycle_publishes_expected_event_sequence(event_bus, risk_engine):
    ...
    for topic in ["decision.opportunity", "decision.entry", "trade.order_submitted", "trade.order_filled", "trade.position_opened"]:
    ...
    assert set(received_topics) == {"decision.opportunity", "decision.entry", "trade.order_submitted", "trade.order_filled", "trade.position_opened"}
...
async def test_failed_order_submission_rejects_trade_instead_of_opening_phantom_position(event_bus, risk_engine):
    ...
    class FailingExecutor(OrderExecutor):
        async def submit_order(self, request):
            return OrderResult(order_id="failed-1", symbol=request.symbol, side=request.side, filled_quantity=0.0, fill_price=0.0, success=False, error="insufficient margin")
...
async def test_existing_sector_hard_cap_rejects_candidate_before_order(event_bus, risk_engine):
    ...
    portfolio = make_portfolio(
        ...
        positions=(PositionExposure(symbol="BNBUSDT", notional_usd=4100.0, leverage=1.0),),
    )
