...
SYMBOLS = ["BTCUSDT", "ETHUSDT", "SOLUSDT", "BNBUSDT"]; SCAN_INTERVAL_SECONDS = 60.0; KLINE_TIMEFRAME = "15m"; STARTING_EQUITY_USD = 10_000.0; HEALTH_SERVER_PORT = 8090
...
async def try_connect_clickhouse_repositories(settings) -> tuple[Optional[MarketDataRepository], Optional[JournalRepository]]:
    ...
    except Exception as exc: logger.warning("ClickHouse unavailable: %s", exc); return None, None
...
async def main() -> None:
    settings = get_settings(); configure_logging(settings.log_level); redis_client = await connect_redis_with_retry(settings)
