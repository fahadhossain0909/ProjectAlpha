...
SCAN_INTERVAL_SECONDS = 60.0; KLINE_TIMEFRAME = "15m"; STARTING_EQUITY_USD = 10_000.0; HEALTH_SERVER_PORT = 8090
...
async def try_connect_clickhouse_repositories(settings) -> tuple[Optional[MarketDataRepository], Optional[JournalRepository]]:
    ...
    try:
        await market_repo.initialize({}); await journal_repo.initialize({}); return market_repo, journal_repo
    except Exception as exc:
        logger.warning("ClickHouse unavailable, running without persistence: %s", exc); return None, None
...
async def main() -> None:
    ...
    components = await build_system(event_bus=event_bus, exchange=BinanceFuturesAdapter(), order_executor=SimulatedOrderExecutor(), symbols=SYMBOLS,
                                    ...
                                    risk_limits=None, rl_scorer=DeepValueRLScorer())
    ...
    try:
        ...
        await health_server.stop(); await experience_recorder.shutdown(); await shutdown_all(components)
