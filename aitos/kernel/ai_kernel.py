...
class AIKernel(AITOSModule):
    def __init__(self, event_bus: EventBus, require_human_approval_for_prod: bool = True,
                 fusion_engine: Optional[DecisionFusionEngine] = None) -> None:
    ...
    @property
    def module_id(self) -> str: return "ai-kernel"
    @property
    def version(self) -> str: return "1.1.1"
    @property
    def fusion_min_confidence(self) -> float: return self._fusion_engine.min_confidence
    ...
    async def health_check(self) -> HealthStatus:
        return HealthStatus(module_id=self.module_id,
            ...
            details={"registered_agents": list(self._agents.keys()), "fusion_min_confidence": self.fusion_min_confidence})
    ...
    async def handle_event(self, event: Event) -> Optional[EventResponse]:
        ...
        for agent in self._agents.values(): await agent.handle_event(event)
        return None
    ...
    async def request_decision(self, context: DecisionContext) -> FusedDecision:
        ...
        if evidence is not None:
            ...
            return FusedDecision(symbol=context.symbol, direction=evidence.direction, confidence=evidence.confidence,
                contributions=contributions,
                conflicting_evidence=([f"insufficient evidence: {', '.join(evidence.missing_components)}"]
                    if evidence.direction == "neutral" and evidence.missing_components else []))
        ...
        return FusedDecision(symbol=context.symbol, direction=fused_direction, confidence=round(min(fused_confidence, 1.0), 4),
            contributions=[d.to_dict() for d in decisions], conflicting_evidence=conflicting)
    ...
    async def enforce_governance(self, action: Action) -> GovernanceResult:
        ...
        if action.is_production and self._require_human_approval_for_prod and not action.approved_by:
            return GovernanceResult(False, "Production action requires explicit human approval (approved_by is empty).", True)
