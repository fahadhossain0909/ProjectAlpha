...
class DeepValueRLScorer(RLPolicyScorer):
    ...
    def update_and_persist(self, symbol: str, context: Dict[str, Any], reward_r_multiple: float) -> None:
    ...
    def save_state(self, path: str | None = None) -> None:
        ...
        with tmp.open("wb") as handle:
            pickle.dump({"model": self._model, "n_samples_seen": self._n_samples_seen, "recent_rewards": self._recent_rewards}, handle, protocol=pickle.HIGHEST_PROTOCOL)
        tmp.replace(target)
