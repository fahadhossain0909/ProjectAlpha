...
class FlowLiquidityInteractionEngine:
    ...
    def evaluate(
        ...
        ask_sweep = max((e.score for e in events if e.kind == "sweep" and e.side == "ask"), default=0.0)
        bid_sweep = max((e.score for e in events if e.kind == "sweep" and e.side == "bid"), default=0.0)
        ask_pull = max((e.score for e in events if e.kind == "pulling" and e.side == "ask"), default=0.0)
        bid_pull = max((e.score for e in events if e.kind == "pulling" and e.side == "bid"), default=0.0)
        ask_stack = max((e.score for e in events if e.kind == "stacking" and e.side == "ask"), default=0.0)
        bid_stack = max((e.score for e in events if e.kind == "stacking" and e.side == "bid"), default=0.0)

        # Aggressive buying + ask replenishment/stacking is a conservative absorption proxy.
        if footprint.bias == "bullish" and (ask_stack > 0 or footprint.absorption_score >= 6.0):
            ...
            return FlowLiquiditySignal("buy_absorption_proxy", "bearish", score, tuple(evidence))

        # Aggressive selling + bid replenishment/stacking is a conservative absorption proxy.
        if footprint.bias == "bearish" and (bid_stack > 0 or footprint.absorption_score >= 6.0):
            ...
            return FlowLiquiditySignal("sell_absorption_proxy", "bullish", score, tuple(evidence))

        # Sweep direction follows aggressive flow only when both signals agree.
        if ask_sweep >= 5.0 and footprint.bias == "bullish":
            score = min(10.0, (ask_sweep + footprint.delta_score + footprint.imbalance_score) / 3.0)
            evidence.append("aggressive buying coincided with ask-liquidity removal")
            return FlowLiquiditySignal("buy_side_sweep", "bullish", score, tuple(evidence))

        if bid_sweep >= 5.0 and footprint.bias == "bearish":
            score = min(10.0, (bid_sweep + (10.0 - footprint.delta_score) + (10.0 - footprint.imbalance_score)) / 3.0)
            evidence.append("aggressive selling coincided with bid-liquidity removal")
            return FlowLiquiditySignal("sell_side_sweep", "bearish", score, tuple(evidence))
