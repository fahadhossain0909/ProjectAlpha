...
class LiquidityTracker:
    ...
    def _detect_sweep(self, previous: OrderBookSnapshot, current: OrderBookSnapshot, trades: Sequence[TradeTick]) -> list[LiquidityEvent]:
        ...
        if prev_bid_qty > 0 and curr_bid_qty / prev_bid_qty < 1.0 - self.removal_ratio and sell_qty > 0:
            score = min(10.0, (1.0 - curr_bid_qty / prev_bid_qty) * 10.0 + min(5.0, sell_qty / max(prev_bid_qty, 1e-12) * 5.0))
            events.append(LiquidityEvent("sweep", "bid", round(score, 2), current.best_bid, "bid liquidity removed with aggressive selling"))
        return events
