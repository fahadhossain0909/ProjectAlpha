...
DEFAULT_WEIGHTS: Dict[str, float] = {
    ...
    "lead_lag": 0.10, "funding_rate": 0.10, "open_interest_trend": 0.10,
    "rl_confidence": 0.05,
}
...
class OpportunityScanner(AITOSModule):
    def __init__(self, event_bus: EventBus, exchange: ExchangeAdapter, symbols: List[str], timeframe: str = "15m",
                 reference_symbol: str = "BTCUSDT", rl_scorer: Optional[RLPolicyScorer] = None,
                 weights: Optional[Dict[str, float]] = None, min_score_threshold: float = 60.0,
                 top_n: int = 5, kline_lookback: int = 100) -> None:
        self._event_bus, self._exchange, self._symbols = event_bus, exchange, symbols
        self._timeframe, self._reference_symbol = timeframe, reference_symbol
        self._rl_scorer = rl_scorer or NeutralRLScorer()
        self._weights = weights or DEFAULT_WEIGHTS
        self._min_score_threshold, self._top_n, self._kline_lookback = min_score_threshold, top_n, kline_lookback
    ...
    async def health_check(self) -> HealthStatus:
        return HealthStatus(module_id=self.module_id,
            ...
            latency_ms=0.0, last_event_time=self._last_scan_at,
    ...
    async def scan_symbol(self, symbol: str, reference_klines: Optional[list] = None) -> Optional[ScanCandidate]:
        ...
        rl_context = {"regime": regime, "direction": direction.value, "trend_strength": round(trend_score, 2),
                      "liquidity_quality": liquidity_score, "order_flow_bias": cvd_score,
                      "auction_context": structure_strength, "volatility": volatility_score,
                      "market_regime": regime_score, "lead_lag": lead_lag, "funding_rate": funding_score,
        ...
        component_scores = {"trend_strength": round(trend_score, 2), "liquidity_quality": liquidity_score,
            "order_flow_bias": cvd_score, "auction_context": structure_strength, "volatility": volatility_score,
            "market_regime": regime_score, "lead_lag": lead_lag, "funding_rate": funding_score,
        ...
        return ScanCandidate(symbol=symbol, direction=direction, composite_score=round(composite, 2),
            component_scores=component_scores, rationale=rationale, entry_price=klines[-1].close, atr=atr, regime=regime)
    ...
    async def scan_all(self) -> List[ScanCandidate]:
        ...
        await self._event_bus.publish(Event(topic=TOPIC_SCAN_COMPLETE,
            payload={"symbols_scanned": len(self._symbols), "candidates_found": len(candidates)}, source_module=self.module_id))
        return candidates
    ...
    async def rank(self, candidates: List[ScanCandidate], top_n: Optional[int] = None) -> List[ScanCandidate]:
        ...
        return sorted([c for c in candidates if c.composite_score >= self._min_score_threshold],
                      key=lambda c: c.composite_score, reverse=True)[:effective_top_n]

    async def decide_with_kernel(self, candidate: ScanCandidate, kernel: Any) -> Any:
        """Send the scanner's actual evidence to the AI Kernel.

        This is the explicit seam between market-data scoring and central
        decision fusion. It does not bypass risk or execution governance.
        """
        from aitos.kernel.ai_kernel import DecisionContext
        return await kernel.request_decision(DecisionContext(
            symbol=candidate.symbol,
            context={"direction": candidate.direction.value, "component_scores": candidate.component_scores,
                     "regime": candidate.regime, "entry_price": candidate.entry_price},
        ))

    def to_opportunity(self, candidate: ScanCandidate, risk_reward_multiples: Tuple[float, ...] = (1.0, 2.0, 3.0),
                       atr_stop_multiplier: float = 1.5, strategy_id: str = "opportunity-scanner",
                       is_production: bool = False, approved_by: Optional[str] = None) -> Opportunity:
        ...
        if candidate.direction == TradeSide.LONG:
            ...
            stop_loss_price, take_profit_levels = entry + stop_distance, [entry - stop_distance * r for r in risk_reward_multiples]
        return Opportunity(symbol=candidate.symbol, side=candidate.direction, entry_price=entry,
            stop_loss_price=stop_loss_price, take_profit_levels=take_profit_levels,
            confidence=round(candidate.composite_score / 100.0, 4), strategy_id=strategy_id,
            rationale="; ".join(candidate.rationale), agent_consensus=dict(candidate.component_scores),
            is_production=is_production, approved_by=approved_by, trailing_sl_enabled=True, regime=candidate.regime)
