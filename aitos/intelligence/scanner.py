...
class OpportunityScanner(AITOSModule):
    def __init__(self, event_bus: EventBus, exchange: ExchangeAdapter, symbols: List[str],
                 timeframe: str = "15m", reference_symbol: str = "BTCUSDT",
                 rl_scorer: Optional[RLPolicyScorer] = None, weights: Optional[Dict[str, float]] = None,
                 min_score_threshold: float = 60.0, top_n: int = 5, kline_lookback: int = 100,
                 trade_lookback: int = 500, footprint_tick_sizes: Optional[Dict[str, float]] = None,
                 live_state_stale_seconds: float = 5.0, amt_value_area_pct: float = 0.70,
                 amt_ib_minutes: int = 60) -> None:
        ...
        self._last_amt_profile: Dict[str, Any] = {}
        self._last_amt_context: Dict[str, AMTContext] = {}
        self._liquidity_trackers: Dict[str, LiquidityTracker] = {}
        self._footprint_engines: Dict[str, FootprintEngine] = {}
        self._amt_engines: Dict[str, AMTEngine] = {}
        self._footprint_signal_engine = FootprintSignalEngine()
        self._interaction_engine = FlowLiquidityInteractionEngine()
        self._live_cache = LiveScannerCache(event_bus, symbols, max_trades=max(5000, trade_lookback))
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        if missing:
            try:
                ...
                logger.info("loaded footprint tick sizes", extra={"aitos_extra": {
                    "symbols": list(filters),
                    "missing": [s for s in missing if s not in self._footprint_tick_sizes],
                }})
            except Exception as exc:
                logger.warning("could not auto-load footprint tick sizes; footprint remains neutral for missing symbols",
    ...
    async def health_check(self) -> HealthStatus:
        return HealthStatus(
            ...
            details={
                ...
                "live_state_symbols": sum(1 for s in self._symbols if self._live_cache.snapshot(s) is not None),
            },
        )
    ...
    def _live_market_data(self, symbol: str) -> tuple[list, Any, bool]:
        ...
        trade_age = (now - state.last_trade_at).total_seconds()
        book_age = (now - state.last_book_at).total_seconds()
    ...
    def _analyze_amt(self, symbol: str, trades: list, klines: list, order_book: Any) -> Optional[AMTContext]:
        ...
        previous = self._last_amt_profile.get(symbol)
    ...
    async def scan_symbol(self, symbol: str, reference_klines: Optional[list] = None) -> Optional[ScanCandidate]:
        ...
        if live_fresh and live_trades and live_book is not None:
            ...
            trades = await self._exchange.fetch_recent_trades(symbol, limit=self._trade_lookback)
        ...
        lead_lag = indicators.lead_lag_score(klines, reference_klines) if reference_klines and symbol != self._reference_symbol else 5.0
        ...
        live_auction = live_auction_score(trades, order_book, direction.value) if live_fresh else 5.0
        auction_score = live_auction if live_fresh else candle_auction
        amt_context = self._analyze_amt(symbol, trades, klines, order_book)
        if amt_context is not None:
            # Structured AMT is authoritative when executable trade-by-price data is available.
            # Keep the legacy score as a compatibility signal rather than deleting it abruptly.
            location_score = 5.0 + (amt_context.price_location - 0.5) * 4.0
        ...
        if tick_size is not None and trades:
            footprint_engine = self._footprint_engines.setdefault(symbol, FootprintEngine(tick_size))
            footprint = footprint_engine.build(trades)
            footprint_signals = self._footprint_signal_engine.evaluate(footprint)
            interaction = self._interaction_engine.evaluate(footprint_signals, liquidity_events)
            interaction_score = (5.0 if interaction.direction == "neutral" else
                                 5.0 + interaction.score * 0.5 if interaction.direction == direction.value else
                                 max(0.0, 5.0 - interaction.score * 0.5))
        ...
        rl_context = {
            "regime": regime, "direction": direction.value,
            "trend_strength": trend_score, "liquidity_quality": liquidity_score,
            "order_flow_bias": flow_score, "auction_context": amt_score,
            "volatility": volatility_score, "market_regime": regime_score,
            "lead_lag": lead_lag, "funding_rate": funding_score,
            "open_interest_trend": oi_score, "footprint_interaction": interaction_score,
        }
        if amt_context is not None:
            rl_context.update({
        ...
        component_scores = {
            "trend_strength": round(trend_score, 2), "liquidity_quality": liquidity_score,
            "order_flow_bias": flow_score, "auction_context": amt_score,
            "volatility": volatility_score, "market_regime": regime_score,
            "lead_lag": lead_lag, "funding_rate": funding_score,
            "open_interest_trend": oi_score, "rl_confidence": round(rl_score, 2),
            "footprint_interaction": interaction_score,
        }
        weight_total = sum(self._weights.get(k, 0.0) for k in component_scores)
        composite = sum(component_scores[k] * self._weights.get(k, 0.0) for k in component_scores) / weight_total * 10 if weight_total else 0.0
        rationale = [f"{k.replace('_', ' ')}={v:.1f}/10" for k, v in component_scores.items()]
        rationale.append(
            f"executed_trades={len(trades)}, candle_cvd={candle_cvd:.1f}, live_auction={live_auction:.1f}, "
            f"candle_auction={candle_auction:.1f}, amt_score={amt_score:.1f}, regime={regime}, "
            f"structure={structure_direction}, direction={direction.value}, market_source={market_source}"
        )
        if amt_context is not None:
            rationale.extend([
        ...
        if flow_features:
            rationale.append(f"orderflow delta={flow_features.delta:.4f}, cvd={flow_features.cvd:.4f}, buy_ratio={flow_features.buy_ratio:.3f}, aggression={flow_features.aggression:.3f}, vwap={flow_features.vwap:.4f}")
        rationale.append(interaction_rationale)
    ...
    async def scan_all(self) -> List[ScanCandidate]:
        ...
        if self._reference_symbol:
            try:
                reference_klines = await self._exchange.fetch_klines(self._reference_symbol, self._timeframe, limit=self._kline_lookback)
            except Exception as exc:
        ...
        await self._event_bus.publish(Event(topic=TOPIC_SCAN_COMPLETE, payload={"symbols_scanned": len(self._symbols), "candidates_found": len(candidates)}, source_module=self.module_id))
        return candidates
    ...
    async def rank(self, candidates: List[ScanCandidate], top_n: Optional[int] = None) -> List[ScanCandidate]:
        ...
        return sorted([c for c in candidates if c.composite_score >= self._min_score_threshold], key=lambda c: c.composite_score, reverse=True)[:n]
    ...
    def to_opportunity(self, candidate: ScanCandidate, risk_reward_multiples: Tuple[float, ...] = (1.0, 2.0, 3.0), atr_stop_multiplier: float = 1.5, strategy_id: str = "opportunity-scanner", is_production: bool = False, approved_by: Optional[str] = None) -> Opportunity:
        ...
        if candidate.direction == TradeSide.LONG:
            stop_loss_price, take_profit_levels = entry - stop_distance, [entry + stop_distance * r for r in risk_reward_multiples]
        else:
            stop_loss_price, take_profit_levels = entry + stop_distance, [entry - stop_distance * r for r in risk_reward_multiples]
