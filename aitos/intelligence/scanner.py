...
class OpportunityScanner(AITOSModule):
    def __init__(self, event_bus: EventBus, exchange: ExchangeAdapter, symbols: List[str], timeframe: str = "15m", reference_symbol: str = "BTCUSDT", rl_scorer: Optional[RLPolicyScorer] = None, weights: Optional[Dict[str, float]] = None, min_score_threshold: float = 60.0, top_n: int = 5, kline_lookback: int = 100, trade_lookback: int = 500, footprint_tick_sizes: Optional[Dict[str, float]] = None, live_state_stale_seconds: float = 5.0, amt_value_area_pct: float = 0.70, amt_ib_minutes: int = 60) -> None:
        self._event_bus=event_bus; self._exchange=exchange; self._symbols=symbols; self._timeframe=timeframe; self._reference_symbol=reference_symbol; self._rl_scorer=rl_scorer or NeutralRLScorer(); self._weights=weights or DEFAULT_WEIGHTS; self._min_score_threshold,self._top_n=min_score_threshold,top_n; self._kline_lookback,self._trade_lookback=kline_lookback,trade_lookback; self._footprint_tick_sizes=footprint_tick_sizes or {}; self._live_state_stale_seconds=max(0.5,live_state_stale_seconds); self._amt_value_area_pct=amt_value_area_pct; self._amt_ib_minutes=amt_ib_minutes; self._initialized=False; self._last_oi={}; self._last_amt_profile={}; self._last_amt_context={}; self._liquidity_trackers={}; self._footprint_engines={}; self._amt_engines={}; self._footprint_signal_engine=FootprintSignalEngine(); self._interaction_engine=FlowLiquidityInteractionEngine(); self._live_cache=LiveScannerCache(event_bus,symbols,max_trades=max(5000,trade_lookback)); self._last_scan_at=None; self._last_candidate_count=0
    @property
    ...
    def _live_market_data(self,symbol:str)->tuple[list,Any,bool]:
        ...
        now=datetime.now(timezone.utc); return list(state.trades),state.order_book,(now-state.last_trade_at).total_seconds()<=self._live_state_stale_seconds and (now-state.last_book_at).total_seconds()<=self._live_state_stale_seconds
    def _analyze_amt(self,symbol:str,trades:list,klines:list,order_book:Any)->Optional[AMTContext]:
        ...
        engine=self._amt_engines.setdefault(symbol,AMTEngine(tick_size,self._amt_value_area_pct,self._amt_ib_minutes)); context=engine.analyze(trades,klines=klines,book=order_book,previous_profile=self._last_amt_profile.get(symbol)); self._last_amt_profile[symbol]=context.profile; self._last_amt_context[symbol]=context; return context
