...
DEFAULT_WEIGHTS: Dict[str, float] = {
    "trend_strength": 0.10, "liquidity_quality": 0.10,
    "order_flow_bias": 0.15, "auction_context": 0.10,
    "volatility": 0.05, "market_regime": 0.10, "lead_lag": 0.10,
    "funding_rate": 0.08, "open_interest_trend": 0.08,
    "rl_confidence": 0.04, "footprint_interaction": 0.10,
}
...
class OpportunityScanner(AITOSModule):
    def __init__(self, event_bus: EventBus, exchange: ExchangeAdapter, symbols: List[str], timeframe: str = "15m", reference_symbol: str = "BTCUSDT", rl_scorer: Optional[RLPolicyScorer] = None, weights: Optional[Dict[str, float]] = None, min_score_threshold: float = 60.0, top_n: int = 5, kline_lookback: int = 100, trade_lookback: int = 500, footprint_tick_sizes: Optional[Dict[str, float]] = None, live_state_stale_seconds: float = 5.0, amt_value_area_pct: float = 0.70, amt_ib_minutes: int = 60) -> None:
        self._event_bus = event_bus; self._exchange = exchange; self._symbols = symbols; self._timeframe = timeframe; self._reference_symbol = reference_symbol
        self._rl_scorer = rl_scorer or NeutralRLScorer(); self._weights = weights or DEFAULT_WEIGHTS; self._min_score_threshold, self._top_n = min_score_threshold, top_n
        self._kline_lookback, self._trade_lookback = kline_lookback, trade_lookback; self._footprint_tick_sizes = footprint_tick_sizes or {}; self._live_state_stale_seconds = max(0.5, live_state_stale_seconds)
        self._amt_value_area_pct = amt_value_area_pct; self._amt_ib_minutes = amt_ib_minutes; self._initialized = False; self._last_oi: Dict[str, OpenInterest] = {}; self._last_amt_profile: Dict[str, Any] = {}; self._last_amt_context: Dict[str, AMTContext] = {}
        self._liquidity_trackers: Dict[str, LiquidityTracker] = {}; self._footprint_engines: Dict[str, FootprintEngine] = {}; self._amt_engines: Dict[str, AMTEngine] = {}; self._footprint_signal_engine = FootprintSignalEngine(); self._interaction_engine = FlowLiquidityInteractionEngine()
        self._live_cache = LiveScannerCache(event_bus, symbols, max_trades=max(5000, trade_lookback)); self._last_scan_at: Optional[str] = None; self._last_candidate_count = 0
    @property
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        if missing:
            try:
                ...
                logger.warning("could not auto-load footprint tick sizes", extra={"aitos_extra": {"error": str(exc), "symbols": missing}})
    ...
    def _live_market_data(self, symbol: str) -> tuple[list, Any, bool]:
        ...
        return list(state.trades), state.order_book, trade_age <= self._live_state_stale_seconds and book_age <= self._live_state_stale_seconds
    def _analyze_amt(self, symbol: str, trades: list, klines: list, order_book: Any) -> Optional[AMTContext]:
        tick_size = self._footprint_tick_size(symbol)
        if tick_size is None or not trades: return None
        engine = self._amt_engines.setdefault(symbol, AMTEngine(tick_size, self._amt_value_area_pct, self._amt_ib_minutes)); previous = self._last_amt_profile.get(symbol)
        context = engine.analyze(trades, klines=klines, book=order_book, previous_profile=previous); self._last_amt_profile[symbol] = context.profile; self._last_amt_context[symbol] = context; return context
    async def scan_symbol(self, symbol: str, reference_klines: Optional[list] = None) -> Optional[ScanCandidate]:
        ...
        funding = await self._exchange.fetch_funding_rate(symbol); oi_current = await self._exchange.fetch_open_interest(symbol); oi_previous = self._last_oi.get(symbol); atr = indicators.average_true_range(klines); vol_percentile = indicators.atr_percentile(klines); regime = indicators.classify_regime(klines); structure_direction, structure_strength = indicators.detect_structure_break(klines)
        flow_features = OrderFlowEngine(max_trades=max(100, self._trade_lookback)).ingest_many(trades) if trades else None; candle_cvd = indicators.cvd_trend_score(klines); flow_score = flow_features.bias_score if flow_features else candle_cvd; direction = determine_direction(structure_direction, flow_score); self._last_oi[symbol] = oi_current
        ...
        candle_auction = auction_context_score(klines, direction.value); live_auction = live_auction_score(trades, order_book, direction.value) if live_fresh else 5.0; auction_score = live_auction if live_fresh else candle_auction; amt_context = self._analyze_amt(symbol, trades, klines, order_book)
        if amt_context is not None:
            location_score = 5.0 + (amt_context.price_location - 0.5) * 4.0 if direction == TradeSide.LONG else 5.0 - (amt_context.price_location - 0.5) * 4.0
            state_bias = {"trend": 1.0, "discovery_up": 0.8, "discovery_down": -0.8, "acceptance": 0.5, "rejection": -0.5, "balance": 0.0, "rotation": 0.0, "unknown": 0.0}.get(amt_context.state.value, 0.0)
            if direction == TradeSide.SHORT: state_bias *= -1.0
            amt_score = round(max(0.0, min(10.0, location_score + state_bias + (amt_context.acceptance - amt_context.rejection))), 2)
        else: amt_score = auction_score
        ...
        rl_score = await self._rl_scorer.score(symbol, rl_context); component_scores = {"trend_strength": round(trend_score, 2), "liquidity_quality": liquidity_score, "order_flow_bias": flow_score, "auction_context": amt_score, "volatility": volatility_score, "market_regime": regime_score, "lead_lag": lead_lag, "funding_rate": funding_score, "open_interest_trend": oi_score, "rl_confidence": round(rl_score, 2), "footprint_interaction": interaction_score}
        weight_total = sum(self._weights.get(k, 0.0) for k in component_scores); composite = sum(component_scores[k] * self._weights.get(k, 0.0) for k in component_scores) / weight_total * 10 if weight_total else 0.0
        rationale = [f"{k.replace('_', ' ')}={v:.1f}/10" for k, v in component_scores.items()]
        rationale.append(f"executed_trades={len(trades)}, candle_cvd={candle_cvd:.1f}, live_auction={live_auction:.1f}, candle_auction={candle_auction:.1f}, amt_score={amt_score:.1f}, regime={regime}, structure={structure_direction}, direction={direction.value}, market_source={market_source}")
        ...
        rationale.append(interaction_rationale); rationale.append(f"liquidity=orderbook+tradeflow, structure_strength={structure_strength:.1f}"); rationale.append(f"regime={regime}")
    ...
    async def scan_all(self) -> List[ScanCandidate]:
        ...
        for symbol in self._symbols:
            try:
                ...
                if candidate is not None and candidate.composite_score >= self._min_score_threshold: candidates.append(candidate)
            except Exception as exc: logger.exception("scan failed", extra={"aitos_extra": {"symbol": symbol, "error": str(exc)}})
        self._last_scan_at = _utc_now_iso(); self._last_candidate_count = len(candidates); return candidates
    async def rank(self, candidates: List[ScanCandidate]) -> List[ScanCandidate]: return sorted(candidates, key=lambda c: c.composite_score, reverse=True)[: self._top_n]
    def to_opportunity(self, candidate: ScanCandidate, risk_reward_multiples: Tuple[float, ...] = (1.0, 2.0, 3.0), atr_stop_multiplier: float = 1.5) -> Opportunity:
        stop_distance = max(candidate.atr * atr_stop_multiplier, candidate.entry_price * 0.001); stop = candidate.entry_price - stop_distance if candidate.direction == TradeSide.LONG else candidate.entry_price + stop_distance; tps = tuple(candidate.entry_price + stop_distance * r if candidate.direction == TradeSide.LONG else candidate.entry_price - stop_distance * r for r in risk_reward_multiples); return Opportunity(symbol=candidate.symbol, side=candidate.direction, entry_price=candidate.entry_price, stop_loss_price=stop, take_profit_levels=tps, confidence=max(0.0, min(1.0, candidate.composite_score / 100.0)), rationale=candidate.rationale, metadata={"regime": candidate.regime, "component_scores": candidate.component_scores, "scanner_version": self.version, "trailing_sl_enabled": True})
