"""Deterministic in-memory multi-session AMT history.

The store is deliberately storage-agnostic: production callers can persist
its snapshots in ClickHouse/Redis without coupling the AMT measurement layer
...
class SessionProfileStore:
    ...
    def upsert(self, snapshot: SessionSnapshot) -> None:
        ...
        if len(self._items) > self.max_sessions:
            oldest = sorted(self._items.values(), key=lambda x: x.start)[0]
