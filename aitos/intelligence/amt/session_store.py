...
class SessionProfileStore:
    ...
    def __init__(self, max_sessions: int = 256) -> None:
        if max_sessions <= 0: raise ValueError("max_sessions must be > 0")
    ...
    def get(self, session_id: str) -> SessionSnapshot | None: return self._items.get(session_id)
    ...
    def recent(self, limit: int = 20) -> Sequence[SessionSnapshot]:
        if limit <= 0: return ()
        return tuple(sorted(self._items.values(), key=lambda x: x.start, reverse=True)[:limit])

    def migration(self, session_id: str) -> ValueMigration:
        current = self._items.get(session_id); prev = self.previous(session_id)
        if current is None or prev is None: return ValueMigration.UNKNOWN
