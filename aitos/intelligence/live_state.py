...
class LiveMarketStateStore:
    def __init__(self, max_trades: int = 5000, max_liquidity_events: int = 100) -> None:
        self._trades: Dict[str, Deque[TradeTick]] = defaultdict(lambda: deque(maxlen=max_trades))
        self._flow: Dict[str, OrderFlowEngine] = defaultdict(lambda: OrderFlowEngine(max_trades=max_trades))
    ...
    def on_order_book(self, book: OrderBookSnapshot) -> tuple[LiquidityEvent, ...]:
        events = tuple(self._liquidity[book.symbol].update(book, tuple(self._trades[book.symbol])))
        self._events[book.symbol].extend(events)
        self._books[book.symbol] = book
        return events

    def snapshot(self, symbol: str) -> LiveMarketState:
        return LiveMarketState(self._flow[symbol].snapshot(), tuple(self._events[symbol]), self._books.get(symbol), len(self._trades[symbol]))
