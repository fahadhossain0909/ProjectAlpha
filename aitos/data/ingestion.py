...
def live_state_topic(symbol: str) -> str: return f"market.live_state.{symbol}"
...
class DataIngestionService(AITOSModule):
    ...
    async def _handle_order_book(self, book: OrderBookSnapshot) -> None:
        ...
        self._publish_live_state(book.symbol); self._tick_processed()
    ...
    def _tick_processed(self) -> None: self._ticks_processed += 1; self._last_event_time = datetime.now(timezone.utc).isoformat()
