...
"""
ALL_DDL = [CREATE_MARKET_OHLCV, CREATE_ORDER_BOOK_SNAPSHOTS, CREATE_TRADE_TICKS,
           CREATE_FUNDING_RATES, CREATE_OPEN_INTEREST, CREATE_LEARNING_EXPERIENCES]


class MarketDataRepository(AITOSModule):
    """Persistent data layer used by paper/live and future historical replay."""
    def __init__(self, host: str = "localhost", port: int = 8123, username: str = "default",
                 password: str = "", database: str = "aitos") -> None:
    ...
    @property
    def module_id(self) -> str: return "market-data-repository"
    @property
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        for ddl in ALL_DDL: await self._client.command(ddl)
        self._initialized = True
        logger.info("MarketDataRepository initialized (market + learning tables ensured)")
    ...
    async def health_check(self) -> HealthStatus:
        ...
        return HealthStatus(module_id=self.module_id, status=status,
                            ...
                            last_event_time=self._last_event_time, details={})
    ...
    async def save_learning_experience(self, record: ExperienceRecord) -> None:
        self._require_initialized()
        await self._client.insert("learning_experiences", [[
            record.experience_id, record.timestamp, record.source, record.symbol, record.decision,
            record.outcome, record.reward, record.confidence, record.quantity, record.price,
            ...
            record.strategy_version, record.model_version,
            json.dumps(record.metadata, sort_keys=True, default=str),
        ]], column_names=["experience_id", "timestamp", "source", "symbol", "decision", "outcome", "reward", "confidence", "quantity", "price", "features_json", "market_state_json", "risk_state_json", "strategy_version", "model_version", "metadata_json"])
