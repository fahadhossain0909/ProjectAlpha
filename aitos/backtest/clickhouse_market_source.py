...
class ClickHouseMarketEventSource:
    ...
    def _bounds(self, start: Optional[datetime], end: Optional[datetime]) -> tuple[str, dict]:
    ...
    def events(self, symbol: str, start: Optional[datetime] = None,
               end: Optional[datetime] = None, limit: int = 10_000_000
        ...
        for row in book.result_rows:
            ...
            bids_data = json.loads(bids_raw or "[]") if isinstance(bids_raw, str) else bids_raw
            asks_data = json.loads(asks_raw or "[]") if isinstance(asks_raw, str) else asks_raw
            bids = tuple((float(x["price"] if isinstance(x, dict) else x[0]),
                          float(x["qty"] if isinstance(x, dict) else x[1])) for x in bids_data)
            asks = tuple((float(x["price"] if isinstance(x, dict) else x[0]),
                          float(x["qty"] if isinstance(x, dict) else x[1])) for x in asks_data)
            events.append(OrderBookSnapshot(
                symbol=symbol, bids=bids, asks=asks,
                last_update_id=int(update_id), timestamp=_timestamp(time),
            ))
