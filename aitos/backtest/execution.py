...
class ExecutionSimulator:
    def __init__(self, initial_cash: float, fee_rate: float = 0.0004, slippage_bps: float = 0.0) -> None:
        ...
        if fee_rate < 0 or slippage_bps < 0: raise ValueError("fee_rate and slippage_bps must be non-negative")
    ...
    def execute(self, side: Side, quantity: float, market_price: float) -> Fill:
        if quantity <= 0 or market_price <= 0: raise ValueError("quantity and market_price must be positive")
        ...
        if old == 0 or (old > 0 and signed > 0) or (old < 0 and signed < 0):
            self.avg_entry = ((abs(old) * self.avg_entry) + (abs(signed) * price)) / abs(new) if new else 0.0
        else:
            closing = min(abs(old), abs(signed))
            direction = 1 if old > 0 else -1
            self.realized_pnl += closing * (price - self.avg_entry) * direction
            if new != 0 and (old > 0) != (new > 0): self.avg_entry = price
    ...
    def snapshot(self, mark_price: float) -> PortfolioSnapshot:
        ...
        return PortfolioSnapshot(self.cash, self.position_qty, self.avg_entry, self.realized_pnl + unrealized, self.fees, equity)
