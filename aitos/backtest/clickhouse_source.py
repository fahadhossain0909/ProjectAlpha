...
class ClickHouseHistoricalSource:
    ...
    """
    TABLES = {"ohlcv": "market_ohlcv", "trades": "trade_ticks", "orderbook": "order_book_snapshots"}
    ...
    def events(self, symbol: str, start: Optional[datetime] = None, end: Optional[datetime] = None,
               table: str = "ohlcv", timeframe: str = "15m", limit: int = 10_000_000) -> Iterator[HistoricalEvent]:
        ...
        if table == "ohlcv":
            ...
            sql = "SELECT time, open, high, low, close, volume, quote_volume, trades_count FROM market_ohlcv WHERE " + " AND ".join(filters) + " ORDER BY time LIMIT {limit:UInt32}"
        elif table == "trades":
            sql = "SELECT time, price, quantity, side, trade_id, is_buyer_maker FROM trade_ticks WHERE " + " AND ".join(filters) + " ORDER BY time LIMIT {limit:UInt32}"
        else:
            sql = "SELECT time, bid_levels, ask_levels, spread, depth_ratio, last_update_id FROM order_book_snapshots WHERE " + " AND ".join(filters) + " ORDER BY time LIMIT {limit:UInt32}"
