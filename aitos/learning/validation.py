...
class CandidateValidator:
    ...
    def evaluate(
        ...
        if self.policy.require_walk_forward:
            if not events:
                return ValidationResult(False, "no events available for walk-forward validation", candidate, champion_result)
            start = min(e.timestamp for e in events)
            end = max(e.timestamp for e in events)
            # The end boundary is exclusive, so include the final event's day.
            end = end.replace(hour=0, minute=0, second=0, microsecond=0)
            end = end.replace(day=end.day)  # explicit normalization for static type checkers
            if end <= start:
                return ValidationResult(False, "insufficient temporal span for walk-forward validation", candidate, champion_result)
            ...
            if not walk_forward.passed:
                return ValidationResult(False, walk_forward.reason, candidate, champion_result, walk_forward)

        return ValidationResult(True, "candidate passed backtest and validation gates", candidate, champion_result, walk_forward)
