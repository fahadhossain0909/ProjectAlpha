...
class WalkForwardValidator:
    ...
    evolution loop.
    """
    def __init__(self, initial_cash: float = 10_000.0, fee_rate: float = 0.0004,
                 slippage_bps: float = 0.0, min_positive_windows: int = 1) -> None:
    ...
    def evaluate(self, events: Iterable[Any], strategy: Callable, mark_price: Callable,
                 start: datetime, end: datetime, train_days: int = 180,
                 test_days: int = 30, holdout_days: int = 30) -> WalkForwardResult:
        ...
        while cursor + timedelta(days=test_days) <= test_end:
            ...
            if test_events:
                result = BacktestEngine(self.initial_cash, self.fee_rate, self.slippage_bps).run(test_events, strategy, mark_price)
                windows.append(WindowResult(train_start, train_end, cursor, current_test_end, result))
        ...
        holdout = BacktestEngine(self.initial_cash, self.fee_rate, self.slippage_bps).run(holdout_events, strategy, mark_price) if holdout_events else None
        positive = sum(1 for w in windows if w.result.metrics.total_return > 0)
        passed = bool(windows) and positive >= self.min_positive_windows and (holdout is None or holdout.metrics.total_return > 0)
        reason = "walk-forward and locked holdout passed" if passed else "candidate failed walk-forward/holdout requirements"
