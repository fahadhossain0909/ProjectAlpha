...
class PolicyRollbackService:
    def __init__(self, governance: PolicyGovernance, registry: Any, kernel: Any, event_bus: Any = None):
    ...
    def request(self, request_id: str, policy_version: str, reason: str) -> RollbackRequest:
        req = RollbackRequest(request_id, policy_version, reason, datetime.now(timezone.utc).isoformat())
    ...
    def approve_and_execute(self, request_id: str) -> PolicyVersion:
        ...
        self.requests[request_id] = RollbackRequest(req.request_id, req.policy_version, req.reason, req.created_at, True, True)
        self._publish("policy.rollback_executed", {**self.requests[request_id].__dict__, "restored_version": restored.version})
        return restored
