...
def evaluate_weight_candidate(records: Iterable[Mapping[str, Any]], baseline_weights: Mapping[str, float], candidate_weights: Mapping[str, float], *, min_observations: int = 30, min_improvement: float = 0.0) -> ShadowWeightResult:
    ...
    for decision_id, decision in decisions.items():
        ...
        if not isinstance(raw, Mapping):
            continue
        contributions = {str(k): float(v) for k, v in raw.items() if isinstance(v, (int, float))}
        ...
        for outcome in outcomes.get(decision_id, []):
            ...
            if not isinstance(r, (int, float)):
                continue
            bsignal = sum(float(baseline_weights.get(k, 0.0)) * v for k, v in contributions.items())
            csignal = sum(float(candidate_weights.get(k, 0.0)) * v for k, v in contributions.items())
    ...
    reason = "eligible" if eligible else ("insufficient_observations" if observations < min_observations else "candidate_does_not_meet_improvement_threshold")
    return ShadowWeightResult(baseline_score, candidate_score, delta, observations, delta > 0, eligible, reason)
