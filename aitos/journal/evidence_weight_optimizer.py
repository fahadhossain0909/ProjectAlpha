...
class EvidenceWeightOptimizer:
    ...
    def __init__(self, *, min_observations: int = 30, max_relative_change: float = 0.20,
                 min_weight: float = 0.02, max_weight: float = 0.30) -> None:
    ...
    def propose(self, base_weights: Mapping[str, float], performance: Iterable[EvidencePerformance],
                candidate_id: str, base_policy_version: str = "fusion-v1") -> EvidenceWeightCandidate:
        ...
        stats = {p.source: p for p in performance if p.observations >= self.min_observations}
        raw = dict(base)
        ...
        for source, weight in base.items():
            ...
            quality = 0.5 * p.alignment_rate + 0.5 * max(0.0, min(1.0, 0.5 + p.average_r_multiple))
            confidence[source] = min(1.0, p.observations / float(self.min_observations * 5))
            adjustment = 1.0 + self.max_relative_change * ((quality - 0.5) * 2.0) * confidence[source]
        ...
        return EvidenceWeightCandidate(
            ...
            sample_counts={k: (stats[k].observations if k in stats else 0) for k in base},
