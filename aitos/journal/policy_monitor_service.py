...
class PolicyMonitorService(AITOSModule):
    def __init__(self, event_bus: EventBus, active_version: str = "baseline", baseline_avg_r: float = 0.0,
                 window_size: int = 100, min_observations: int = 30, max_degradation: float = 0.20,
                 min_avg_r: float = 0.0) -> None:
    ...
    @property
    def module_id(self) -> str: return "policy-monitor-service"
    @property
    def version(self) -> str: return "1.0.0"

    async def initialize(self, config: Dict[str, Any]) -> None:
        if self._initialized: return
        self._subscription = await self._event_bus.subscribe("journal.outcome_attributed", self.handle_event, group="policy-monitor")
    ...
    async def handle_event(self, event: Event) -> EventResponse | None:
        ...
        if not isinstance(r, (int, float)):
            return None
        self._window.append({"r_multiple": float(r), "decision_id": payload.get("decision_id"), "trade_id": payload.get("trade_id")})
        health = evaluate_policy_health(self._active_version, list(self._window), baseline_avg_r=self._baseline_avg_r,
                                        min_observations=self._min_observations, max_degradation=self._max_degradation,
                                        min_avg_r=self._min_avg_r)
        self._last_health = health.to_dict()
        if health.rollback_recommended:
            await self._event_bus.publish(Event(topic=TOPIC_ROLLBACK_RECOMMENDED, payload=health.to_dict(), source_module=self.module_id))
        return None
