...
from aitos.risk.models import CircuitBreakerState, LimitBreach, PortfolioState, RiskAction, RiskLimits, RiskScoreBreakdown
...
def score_position_risk(portfolio: PortfolioState, limits: RiskLimits) -> Tuple[float, List[str]]:
    ...
    largest_position_pct = (
        ...
        if portfolio.equity_usd > 0 else 0.0
    )
...
def score_market_risk(portfolio: PortfolioState, limits: RiskLimits) -> Tuple[float, List[str]]:
    ...
    return round(min(100.0, vol_score * 0.5 + regime_penalty + correlation_score * 0.2), 2), notes
...
def score_system_risk(portfolio: PortfolioState, limits: RiskLimits) -> Tuple[float, List[str]]:
    ...
    return round(min(error_score * 0.35 + latency_score * 0.15 + freshness_score * 0.2 + accuracy_score * 0.3, 100.0), 2), notes
...
def score_portfolio_risk(portfolio: PortfolioState, limits: RiskLimits) -> Tuple[float, List[str]]:
    ...
    max_sector_score = max((_pct_of_limit(pct, limits.max_sector_exposure_pct) for pct in sector_exposures.values()), default=0.0)
    if max_sector_score > 80:
        notes.append("sector exposure concentrated")
    return round(min(drawdown_score * 0.4 + daily_loss_score * 0.2 + weekly_loss_score * 0.2 + max_sector_score * 0.2, 100.0), 2), notes
...
def check_limits(portfolio: PortfolioState, limits: RiskLimits) -> List[LimitBreach]:
    ...
    def _check(name: str, observed: float, default_limit: float, hard_cap: float) -> None:
    ...
    _check("max_correlated_exposure_pct", portfolio.max_pairwise_correlation * 100, limits.max_correlated_exposure_pct, limits.max_correlated_exposure_hard_cap_pct)
    ...
    _check("min_data_freshness_seconds", portfolio.data_freshness_seconds, limits.min_data_freshness_seconds, limits.min_data_freshness_hard_cap_seconds)
    return breaches


def _absolute_hard_cap_breaches(portfolio: PortfolioState, limits: RiskLimits) -> List[LimitBreach]:
    """Return only breaches of the configured absolute hard caps."""
    breaches: List[LimitBreach] = []
    checks = [
        ("max_drawdown_pct", portfolio.current_drawdown_pct, limits.max_drawdown_hard_cap_pct),
        ("max_leverage", portfolio.max_position_leverage, limits.max_leverage_hard_cap),
        ("max_open_positions", len(portfolio.positions), limits.max_open_positions_hard_cap),
        ("max_risk_per_day_pct", max(0.0, -portfolio.daily_pnl_pct), limits.max_risk_per_day_hard_cap_pct),
        ("max_risk_per_week_pct", max(0.0, -portfolio.weekly_pnl_pct), limits.max_risk_per_week_hard_cap_pct),
        ("max_correlated_exposure_pct", portfolio.max_pairwise_correlation * 100, limits.max_correlated_exposure_hard_cap_pct),
        ("min_data_freshness_seconds", portfolio.data_freshness_seconds, limits.min_data_freshness_hard_cap_seconds),
    ]
    for name, observed, cap in checks:
        if observed > cap:
            breaches.append(LimitBreach(name, cap, observed, True, f"{name} breached HARD CAP: {observed} > {cap}"))
    for sector, pct in portfolio.sector_exposure_pct.items():
        if pct > limits.max_sector_exposure_hard_cap_pct:
            breaches.append(LimitBreach(f"max_sector_exposure_pct[{sector}]", limits.max_sector_exposure_hard_cap_pct, pct, True, f"max_sector_exposure_pct[{sector}] breached HARD CAP: {pct} > {limits.max_sector_exposure_hard_cap_pct}"))
    return breaches


class RiskEngine(AITOSModule):
    def __init__(self, event_bus: EventBus, limits: Optional[RiskLimits] = None, circuit_breaker_cooldown_seconds: float = 300.0) -> None:
    ...
    async def health_check(self) -> HealthStatus:
        ...
        return HealthStatus(module_id=self.module_id, status=status, latency_ms=0.0, last_event_time=self._last_event_time, details={"circuit_breaker_state": self._circuit_breaker.state.value, "last_score": self._last_assessment.total if self._last_assessment else None})
    ...
    async def assess(self, portfolio: PortfolioState) -> RiskScoreBreakdown:
        ...
        breakdown = RiskScoreBreakdown(position_risk=position_risk, market_risk=market_risk, system_risk=system_risk, portfolio_risk=portfolio_risk, total=round(total, 2), action=action, explanation=position_notes + market_notes + system_notes + portfolio_notes)
        self._last_assessment = breakdown
        await self._event_bus.publish(Event(topic="risk.score_update", payload=breakdown.to_dict(), source_module=self.module_id))
        ...
        if action == RiskAction.EMERGENCY_STOP or hard_breaches:
            reason = f"risk score {total:.1f} > {SCORE_EMERGENCY_STOP_THRESHOLD}" if action == RiskAction.EMERGENCY_STOP else f"hard cap breach: {hard_breaches[0].message}"
    ...
    async def trigger_emergency_stop(self, reason: str) -> None:
        ...
        await self._event_bus.publish(Event(topic="risk.emergency_stop", payload={"reason": reason, "circuit_breaker_state": self._circuit_breaker.state.value}, source_module=self.module_id, priority=EventPriority.CRITICAL))
    ...
    async def attempt_recovery(self) -> bool:
        ...
        if transitioned:
            await self._event_bus.publish(Event(topic="risk.circuit_breaker", payload={"state": self._circuit_breaker.state.value, "reason": "cooldown elapsed"}, source_module=self.module_id, priority=EventPriority.HIGH))
        return transitioned
    ...
    def veto(self, portfolio: Optional[PortfolioState] = None) -> Tuple[bool, str]:
        ...
        if self._last_assessment is not None and self._last_assessment.action in (RiskAction.NO_NEW_ENTRIES, RiskAction.EMERGENCY_STOP):
