...
async def try_connect_clickhouse_repositories(settings):
    ...
    try:
        await market_repo.initialize({}); await journal_repo.initialize({})
        return market_repo, journal_repo
    except Exception as exc:
        logger.error("ClickHouse persistence unavailable; live trading continues without durable learning data: %s", exc)
        return None, None
...
async def try_connect_neo4j(settings):
    ...
    try:
        await driver.verify_connectivity(); return driver
    except Exception as exc:
        logger.warning("Neo4j unavailable, running without knowledge graph: %s", exc)
        await driver.close(); return None
...
async def main() -> None:
    ...
    components = await build_system(event_bus=event_bus, exchange=exchange, order_executor=order_executor, symbols=SYMBOLS,
                                     ...
                                     graph_driver=graph_driver, kernel=kernel, use_exchange_side_stops=True)
